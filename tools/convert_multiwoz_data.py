...
def convert_text_for_model(input_file, out_file):
    ...
    for i, s in enumerate(all_samples):
        if len(s['uid']) == 0:
            ...
            del all_samples[i]
    print("max utterances:", max([len(s['uid']) for s in all_samples]))      # 24
    print("min utterances:", min([len(s['uid']) for s in all_samples]))      # 2
    print("avg utterances:", np.mean([len(s['uid']) for s in all_samples]))  # 9.27 / 9.81 / 10.1
    print("max kb triples:", max([len(s['kb']) for s in all_samples]))       # 63 / 61 / 63
    print("min kb triples:", min([len(s['kb']) for s in all_samples]))       # 42
    print("avg kb triples:", np.mean([len(s['kb']) for s in all_samples]))   # 53.45 / 54.68 / 55.26
