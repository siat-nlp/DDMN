    ...
    def forward(self, turn_inputs, kb_inputs, enc_hidden=None):
        ...
        with torch.no_grad():
            ...
            for i, inputs in enumerate(turn_inputs):
                ...
                enc_outputs.add(src=src_text, tgt=tgt_text, preds=preds_text, dialog_id=dialog_id,
                                turn_label=turn_label, task=task,
    ...
    def generate(self, data_iter, output_dir=None, verbos=False):
        ...
        for result_batch in results:
            for result_turn in result_batch:
                ...
                dialog_ids.append(result_turn.dialog_id)
        ...
        for dia, tl, hyp, ref, tk, ent, kb in zip(dialog_ids, turn_labels, hyps, refs, tasks, gold_entity, kb_word):
            hyp_str = " ".join(hyp)
            ref_str = " ".join(ref)
            sample = {"dialog_id": str(dia),
                      "turn_label": str(tl),
                      "result": hyp_str,
                      "target": ref_str,
                      "task": tk,
        ...
        if output_dir is not None:
            ...
            with open("%s/output.txt" % output_dir, 'w', encoding='utf-8') as fw:
                for sample in outputs:
                    line = json.dumps(sample)
                    fw.write(line)
                    fw.write('\n')
            print("Saved generation results to '{}/output.txt.'".format(output_dir))
